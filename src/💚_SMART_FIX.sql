-- ============================================
-- 💚 SMART FIX - الإصلاح الذكي النهائي
-- ============================================
-- يكتشف البنية الحقيقية ويصلح بناءً عليها
...
-- ============================================
-- 3️⃣ دالة التحقق من كود الجلسة (مبسطة)
-- ============================================
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(DISTINCT CASE WHEN s.active = true THEN s.id END)::BIGINT,
        COALESCE(ROUND(AVG(
...
CREATE INDEX IF NOT EXISTS idx_sessions_course_active ON sessions(course_id, active);
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '💚💚💚 SMART FIX COMPLETE! 💚💚💚';
    RAISE NOTICE '';
    ...
    RAISE NOTICE '✅ View: sessions_with_details created';
    RAISE NOTICE '✅ Indexes: created';
    RAISE NOTICE '';
    
    IF v_enrollments_status AND v_attendance_status AND v_attendance_course_id AND v_attendance_method AND v_functions_count = 4 THEN
        RAISE NOTICE '🎊🎊🎊 ALL PERFECT! SYSTEM 100%% READY! 🎊🎊🎊';
    ELSE
        RAISE NOTICE '⚠️ Some items missing, but continuing...';
    END IF;
